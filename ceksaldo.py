...
class HamsterKombat:
    ...
    def process_account(self, data, no, num_acc):
        try:
            ...
            listtask = requests.post("https://api.hamsterkombatgame.io/interlude/sync", headers=headers).json()
            print(f"account {no + 1} token => {listtask['interludeUser']['tokenBalance']['total']}")
